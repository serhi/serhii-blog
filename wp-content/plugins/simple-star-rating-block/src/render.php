...
<span <?php echo get_block_wrapper_attributes(); ?> title="<?php echo esc_attr( $rating ); ?>" style="font-size: <?php echo esc_attr( $star_size ); ?>px;">
	...
	for ( $i = 1; $i <= 5; $i++ ) {
		if ( $i <= $full_stars ) {
			...
			echo '<div class="ssrb-star ssrb-perc-' . esc_attr( $percentage ) . '" style="background-image: linear-gradient(90deg, var(--wp--preset--color--accent) ' . esc_attr( $percentage ) . '%, transparent ' . esc_attr( $percentage ) . '%);"></div>';
		} else {
